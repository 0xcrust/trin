...
type StateEventTx = Option<mpsc::UnboundedSender<OverlayMessage>>;
...
pub async fn initialize_state_network(
    ...
    let (state_event_tx, state_event_rx) = mpsc::unbounded_channel::<OverlayMessage>();
...
pub fn spawn_state_network(
    ...
    portalnet_config: PortalnetConfig,
    state_msg_rx: mpsc::UnboundedReceiver<OverlayMessage>,
) -> JoinHandle<()> {
